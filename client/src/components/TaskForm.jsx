...
export default function TaskForm({ onSubmit, onClose, initialData, isOpen, employees }) {
  ...
  useEffect(() => {
    if (initialData) setForm({ ...initialData, assigned_to: initialData.assigned_to || "" });
    else setForm({ title: "", status: "todo", priority: "medium", due_date: "", assigned_to: "" });
  }, [initialData, isOpen]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm transition-opacity duration-300">
      <div className="bg-gray-900/90 backdrop-blur-md rounded-xl shadow-2xl p-6 w-full max-w-4xl border border-gray-800/50 transform transition-all duration-300 scale-100 hover:scale-[1.01]">
